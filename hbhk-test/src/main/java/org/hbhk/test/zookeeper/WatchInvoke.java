...
public abstract class WatchInvoke {
	...
	/**
	 * 确认是否匹配路径及类型 只有返回TRUE后，才会调用，并且重新注册watch
	 * 
	 * @param path
	 * @return
	 */
	 ...
	 * @return
	 */
	public abstract boolean needContinueWatch(String path, EventType type,
			KeeperState state);
	...
	public void initListen() {
		try {
			Stat stat = zooKeeperOperator.getZk()
					.exists(getListenPath(), false);
			if (stat == null) {
				String data = String.valueOf(System.currentTimeMillis());
				zooKeeperOperator.getZk().create(getListenPath(),
